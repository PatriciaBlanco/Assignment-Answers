...
def add_repeat(bioseq, pos, gene_id, strand)
...
def find_repeats (entry, gene_id)
	
	#Look for CTTCTT repeats in exons and return a Bio::Sequence object with the new CTTCTT repeat features
	
	return false unless entry.seq.match(/cttctt/) or entry.seq.match(/aagaag/)	# If the sequence doesn't even contain the target sequence pass to next gene
	
	bioseq = entry.to_biosequence	# Convert Bio::EMBL object to Bio::Sequence object
	added_pos = [] # Array with the positions of the CTTCTT repeats of the gene
		
	entry.ft do |feature|	
		feature_type = feature.feature
		next unless feature_type == "exon" # If the feature is not an exon pass to next feature
		locations_obj = feature.locations # Bio::Locations object
		locations_obj.each do |location|
			# Bio::Location objects with the position
			...
			when 1 # Forward
				match = exon.scan(/ctt/i) do # Search the motif in the exon 
					if exon[$~.offset(0)[0],6] == "cttctt" #If next nucleotides are ctt save position
						pos = "#{$~.offset(0)[0]+from+1}..#{$~.offset(0)[0]+from+6}"
						unless added_pos.include? pos
							add_repeat(bioseq, pos, gene_id, location.strand)
							added_pos << pos
						end
					end
				end
			...
			when -1 # Complement
				match = entry.seq[from..to].scan(/aag/i) do
					if exon[$~.offset(0)[0],6] == "aagaag"
						pos = "complement(#{$~.offset(0)[0]+from+1}..#{$~.offset(0)[0]+from+6})"
						unless added_pos.include? pos
							add_repeat(bioseq, pos, gene_id, location.strand)
							added_pos << pos
						end
	...
	if added_pos.empty? 
		return false # If no CTTCTT repeat has been found returns false
	else
		return bioseq
	end
end
...
puts "\nSearching CTTCTT repeats...\n\n"
genes = {}			# Hash with all Bio::EMBL objects for the genes in the list (gene_id => Bio::EMBL)
targets = {}		# Hash with the Bio::Sequence::NA objects with cttctt in exons (gene_id => Bio::Sequence::NA)
...
File.readlines(ARGV[0]).each do |gene_id|
	...
	bioseq = find_repeats(entry,gene_id) # Bio::Sequence object with the new cttctt repeat features or false if no repeat was found
...
#Write report with genes without cttctt repeats
...
puts "CTTCTT repeats have been saved in repeats.gff3 (gene coordinates) and chr_repeats.gff3 (chromosome coordinates)"
puts "Genes without CTTCTT repeats in exons have been saved in no_repeats.txt"
